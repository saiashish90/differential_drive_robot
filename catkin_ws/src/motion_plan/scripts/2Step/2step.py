...
initial_position2.x = 2
initial_position2.y = 2
...
def set_action_i(agent):
    ...
    if agent == 1:
            ...
            for i in range(len(borders)):
                utility.append(utility_calc(borders[i],2))
            for i in range (len(borders)):
    ...
    if agent == 2:
            if direction == 1:
                ...
                borders.append(temp3)
            for i in range(len(borders)):
                utility.append(utility_calc(borders[i],1))
            for i in range (len(borders)):
                if borders[i].x <0 or borders[i].y <0 or borders[i].x > grid_size or borders[i].y > grid_sizey:
                    utility[i] = -99
            max = -1
...
def utility_calc(des_position_,agent):
    point = Point()
    u = 0
    point.x =des_position_.x-range_
    point.y =des_position_.y
    if((point.x>=0 and point.y>=0) and (point.x<=grid_size-1 and point.y<=grid_sizey-1) and (explore[point.x][point.y] == 0 or explore[point.x][point.y] == agent) ):
        u= u+1
